fig.r=figure('numbertitle','off','menubar','none','name','PWMs for 1st Phase');
set(fig.r, 'WindowButtonDownFcn','done=1;');
title('VFD With Three Phase Synchronous Motor');
axis equal;


time=0;
dt = 7.0000e-4;  % time span
ripple = 0;   % charge stored in capacitor
A=0.7;         % apmlitude of massage waves in pwm
f.sawtooth =3000;     % freqency of sawtooth wave
...
time = time + dt;
    ...
    %     disp(V.rpwm1)
        ...
        subplot(3,1,1)
        hold on
        plot(time,V.rsawtooth,'black')
        title('Sawtooth Wave 1st Phase')
        ...
        subplot(3,1,2)
        hold on
        plot(time,V.rmassage,'b')
        title('Massage Wave 1st Phase')
        xlabel('Time(sec) \rightarrow')
        ylabel('Amplitude(V) \rightarrow');
        axis([0 0.5 0 2])
        grid on
